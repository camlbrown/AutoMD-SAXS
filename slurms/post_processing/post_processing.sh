...
mkdir final_pdb && cd final_pdb && cp $PDB2GMX_DIR/GMX.gro . && cp ../*ndx .

gmx_mpi editconf -f GMX.gro -n *ndx -o final.pdb << EOF
1
EOF
cp final.pdb ../
...
# C-alpha RMSD
gmx_mpi rms -s final.pdb -f final.xtc  -o rmsd_backbone.xvg -tu ns <<EOF
3
3
EOF

# Gyration
gmx_mpi gyrate -s final.pdb -f final.xtc -o gyrate.xvg <<EOF
System
EOF

# SASA
gmx_mpi sasa -s final.pdb -f final.xtc -o sasa_total.xvg -surface Protein -output Protein
