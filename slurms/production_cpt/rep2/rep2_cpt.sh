...
if [ -e *.gro ]; then
    if grep -q "post_processing initiated" $SIMULATION_DIR/monitor.txt; then
        ...
        echo "repeat2 completed" >> $SIMULATION_DIR/monitor.txt
    ...
    JOBID_MDR2_cpt=$(sbatch --parsable -J r2_cpt --export=ALL "$SLURM_DIR/production_cpt/rep2/rep2_cpt.sh" "$SIMULATION_DIR")
    echo "JOBID_MDR2_cpt=${JOBID_MDR2_cpt}"
fi
