...
def build_variants(topology, pkas, ph):
    """Return ``(variants, audit)`` for ``Modeller.addHydrogens(variants=...)``.

    ``variants`` is a list of length ``n_residues`` (``None`` = OpenMM default,
    else a variant name). ``audit`` records each overridden residue for the
    manifest / UI. Chain ids on ``topology`` must match the propka run
    (call :func:`relabel_chains` before writing the propka input).
    """
